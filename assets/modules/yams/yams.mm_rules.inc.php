      ...
      for ( $i = 0; $i < $nRows; $i++ )
      {
        ...
        switch ( $name )
        {
          case 'pagetitle_' . $langId:
            $standardTVs['1'] = 'pagetitle_' . $langId;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            break;
          case 'longtitle_' . $langId:
            $standardTVs['2'] = 'longtitle_' . $langId;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            break;
          case 'description_' . $langId:
            $standardTVs['3'] = 'description_' . $langId;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            break;
          case 'alias_' . $langId:
            if ( $useMultilingualAliases )
            {
              $standardTVs['4'] = 'alias_' . $langId;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            }
            break;
          case 'introtext_' . $langId:
            $standardTVs['5'] = 'introtext_' . $langId;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            break;
          case 'menutitle_' . $langId:
            $standardTVs['6'] = 'menutitle_' . $langId;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            break;
          case 'content_' . $langId:
            $standardTVs['7'] = 'content_' . $langId;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            break;
          default:
            if ( array_key_exists( $rank, $customTVs) )
            {
              $customTVs[] = $name;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            }
            else
            {
              $customTVs[$rank] = $name;//YAMSTVDataToMMName( $name, $id, 'tv', $mm_version );
            }
        }
      }
