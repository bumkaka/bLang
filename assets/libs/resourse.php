		...
		foreach($fld as $key=>$value){
			if ($value=='') continue;
 			if ($this->tv[$key]!=''){
				...
				if (is_array($row)) {
					if ($row[0] != $value) {
						$result = $this->query("UPDATE {$this->_table['site_tmplvar_contentvalues']} SET `value` = '".$this->modx->db->escape($value)."' WHERE `contentid` = '{$this->id}' AND `tmplvarid` = '{$this->tv[$key]}';");
				    }
				}else{	
					$result = $this->query("INSERT into {$this->_table['site_tmplvar_contentvalues']} SET `contentid` = {$this->id},`tmplvarid` = {$this->tv[$key]},`value` = '".$this->modx->db->escape($value)."';");
				}
			}
		}
